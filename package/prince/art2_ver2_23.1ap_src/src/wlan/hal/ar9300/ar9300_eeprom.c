...
//            printf("Calibration Data is in eeprom\n");
...
#ifdef MDK_AP          
        {
            int fd;
            unsigned short data;
			printf("ar9300CaldataTypeDetect\n");
...
#ifdef MDK_AP          //MDK_AP is defined only in NART AP build 
    int fd;
//	printf("ar9300CalibrationDataReadFlash\n");
    if ((fd = open("/dev/caldata", O_RDWR)) < 0) {
...
//		printf("mdataSize:%x\n",mdataSize);
	...
	if(nptr>=0)
	{
//        printf("mptr->eepromVersion:%x\n",mptr->eepromVersion);
//		printf("mptr->templateVersion:%x\n",mptr->templateVersion);
//        printf(">>>eeprom :  %x %x %x %x %x %x \n",mptr->macAddr[0],mptr->macAddr[1],mptr->macAddr[2]
//                                                 ,mptr->macAddr[3],mptr->macAddr[4],mptr->macAddr[5]);
